...
const HomePage = () => {
    ...
    let movieIDs = movies.map((i) => i.id);
    let detailArr = [];
    ...
    const getDetails = async () => {
        for (let index = 0; index < movieIDs.length; index++) {
            ...
            console.log(data);
            data.genre_ids = data.genres.map((d) => d.id);
            delete data.genres;
            detailArr.push({
                ...data,
                isDeleted: false,
    ...
    useEffect(() => {
        const getMovieList = async () => {
            ...
            try {
                ...
                console.log("abc", res.data?.results);
                // movieIDs=res.data?.results.map()
                setCount(
                    res.data?.total_pages < 500 ? res.data?.total_pages : 500
                );
                setError("");
                    ...
                    {loading ? (
                        ...
                        <>
                            {error ? (
                                ...
                                <>
                                    <AppCarousel
                                        movieList={filterProducts?.slice(0, 10)}
                                    />
